...
const POLICIES = [
  {
    title: "The Zero-Data Protocol",
    code: "APF-001",
    summary: "Complete eradication of targeted data harvesting across all federation assets. Data sovereignty is the baseline requirement for cryptographic identity across all federation nodes.",
...
export function Policies() {
  ...
  const [activeDrafts, setActiveDrafts] = useState(proposedAmendments || []);
  ...
  useEffect(() => {
    ...
    const loadCoreData = async () => {
      try {
        const [proposals, consensusData] = await Promise.all([
        ...
        if (isMounted) {
          ...
          if (consensusData && Object.keys(consensusData).length > 0) {
            const updatedPolicies = POLICIES.map(policy => ({
              ...policy,
              consensus: consensusData[policy.code] || policy.consensus
            }));
            setPolicyData(updatedPolicies);
          }
        }
      } catch (err) {
        if (isMounted) {
  ...
  }, [fetchActiveProposals, fetchPolicyConsensus, addToast, setIsCoreSynced]);
                ...
                {activeDrafts && activeDrafts.length > 0 ? (
                    activeDrafts.map((draft, idx) => (
                        <div key={idx} className="bg-black/40 backdrop-blur-md border border-white/10 shadow-2xl hover:border-apf-purple/40 hover:shadow-[0_0_15px_rgba(148,0,255,0.5)] transition-all duration-500 p-6">
                            ...
                            <h4 className="text-white font-bold text-xl mb-2 font-vt323">{draft.title}</h4>
